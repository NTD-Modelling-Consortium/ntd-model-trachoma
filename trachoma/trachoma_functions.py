...
def _validate_data_path(data_path):
    ...
    if data_path:
        ...
        if not data_path.exists():
            raise FileNotFoundError(f"ERROR: {data_path} does not exists")
        return data_path
...
def readPlatformData(coverageFileName, Platform, data_path=None):
    PlatCov = pd.read_csv(_validate_data_path(data_path) / coverageFileName)
    ...
    if len(PlatformRows) > 0:
            ...
            if len(PlatformS) > 0:
                for k in range(len(PlatformS)):
                    ...
                    if count == 0:
                        PlatformData = [
                            [
                                float(PlatCov.columns[i]),
                                PlatCov.iloc[j, minAgeIndex],
                                PlatCov.iloc[j, maxAgeIndex],
                                PlatCov.iloc[j, i],
                                j,
                                PlatCov.shape[0],
                            ]
        ...
        PlatformData = [[3026.0, 2, 5, 0.6, 0, 2], [3026.0, 2, 5, 0.6, 0, 2]]
    return PlatformData
...
def stepF_fixed(vals, params, demog, bet, distToUse="Poisson"):
    ...
    import_indivs = np.where(
        np.random.uniform(size=params["N"]) < params["importation_rate"]
    )[0]
    ...
    lambda_step = 1 - np.exp(
        -getlambdaStep(
            params=params,
            Age=vals["Age"],
            bact_load=vals["bact_load"],
            IndD=vals["IndD"],
            Infection_risk=vals["Infection_risk"],
            vaccinated=vals["vaccinated"],
            time_since_vaccinated=vals["time_since_vaccinated"],
            bet=bet,
            demog=demog,
        )
    )
    ...
    newDis = np.where(vals["T_latent"] == 1)[
        0
    ]  # Designated latent period for that individual is about to expire
    newClearInf = np.where(vals["T_ID"] == 1)[
        0
    ]  # Designated infectious period for that individual is about to expire
    ...
    vals["T_D"][newClearInf] = D_period_function(
        Ind_D_period_base=vals["Ind_D_period_base"][newClearInf],
        No_Inf=vals["No_Inf"][newClearInf],
        params=params,
        Age=vals["Age"][newClearInf],
    )
...
def get_Intervention_times(Intervention_dates, Start_date, burnin):
    ...
    for i in range(0, len(Intervention_dates)):
        Intervention_times.append(
            burnin + int((Intervention_dates[i] - Start_date).days / 7)
        )
...
def getlambdaStep(
    ...
    o_children = np.where(np.logical_and(Age >= 9 * 52, Age < 15 * 52))[
        0
    ]  # Older children
    ...
    prevLambda = bet * (
        params["v_1"] * totalLoad + params["v_2"] * (totalLoad ** (params["phi"] + 1))
    )
    ...
    prob_reduction = prob_reduction * (
        -time_since_vaccinated / params["vacc_waning_length"] + 1
    )
...
def Reset(Age, demog, params):
    ...
    return np.where(
        np.logical_or(
            np.random.uniform(size=params["N"]) < 1 - np.exp(-demog["tau"]),
            Age > demog["max_age"],
        )
    )[0]
...
def doMDAAgeRange(vals, params, ageStart, ageEnd):
    ...
    if ageStart * 52 <= 26:
        ...
        treated_babies = babies[
            np.where(
                np.random.uniform(size=len(babies)) < vals["treatProbability"][babies]
            )[0]
        ]
        cured_babies = treated_babies[
            np.random.uniform(size=len(treated_babies)) < (params["MDA_Eff"] * 0.5)
        ]

        older = np.where(np.logical_and(Age > 26, Age <= ageEnd * 52))[0]
        treated_older = older[
            np.where(
                np.random.uniform(size=len(older)) < vals["treatProbability"][older]
            )[0]
        ]
        cured_older = treated_older[
            np.random.uniform(size=len(treated_older)) < (params["MDA_Eff"])
        ]
    else:
    ...
    return np.append(cured_babies, cured_older), np.append(
        treated_babies, treated_older
    )
...
def MDA_timestep_Age_range(vals, params, ageStart, ageEnd, t, label, demog):
    ...
    cured_people, treated_people = doMDAAgeRange(
        vals=vals, params=params, ageStart=ageStart, ageEnd=ageEnd
    )
    ...
    vals["n_treatments"][str(mda_t) + ", MDA (campaign " + str(label) + ")"] = (
        treatedAges
    )
...
def doVaccAgeRange(params, vals, vacc_round, VaccData, t, demog):
    ...
    vals["coverageVacc"][VaccData[vacc_round][-2]] += np.count_nonzero(
        vaccInAgeRange
    ) / np.count_nonzero(ageRange)

    vaccAges, _ = np.histogram(
        vals["Age"][vaccInAgeRange] / 52, bins=np.arange(int(demog["max_age"] / 52) + 1)
    )
    vals["n_vaccinated"][
...
def ID_period_function(newDis, params, vals):
    ...
    id_periods = 1 / (
        (1 / Ind_ID_period_base - 1 / params["min_ID"])
        * np.exp(-params["inf_red"] * (No_Inf - 1))
        + 1 / params["min_ID"]
    )
...
def D_period_function(Ind_D_period_base, No_Inf, params, Age):
    ...
    T_D = np.round(
        1
        / (
            (1 / Ind_D_period_base - 1 / params["min_D"])
            * np.exp(-params["dis_red"] * (No_Inf - 1))
            + 1 / params["min_D"]
        )
    )
    return T_D
...
def Set_inits(params, demog, sim_params, MDAData, numpy_state, distToUse="Poisson"):
    ...
    if distToUse == "Poisson":
        Ind_ID_period_base = np.random.poisson(
            lam=params["av_ID_duration"], size=params["N"]
        )

        # Individual's baseline diseased period (first infection)
        Ind_D_period_base = np.random.poisson(
            lam=params["av_D_duration"], size=params["N"]
        )
    else:
        Ind_ID_period_base = np.round(
            np.random.exponential(scale=params["av_ID_duration"], size=params["N"])
        )

        # Individual's baseline diseased period (first infection)
        Ind_D_period_base = np.round(
            np.random.exponential(scale=params["av_D_duration"], size=params["N"])
        )
    ...
    if len(MDAData) > 0:
        ...
        treatProbability = drawTreatmentProbabilities(
            params["N"], MDA_coverage, systematic_non_compliance
        )
    vals = dict(
...
def Reset_vals(vals, reset_indivs, params, distToUse="Poisson"):
    ...
    if distToUse == "Poisson":
        vals["Ind_ID_period_base"][reset_indivs] = np.random.poisson(
            lam=params["av_ID_duration"], size=numResetIndivs
        )
        vals["Ind_D_period_base"][reset_indivs] = np.random.poisson(
            lam=params["av_D_duration"], size=numResetIndivs
        )
    else:
        ID_periods = np.round(
            np.random.exponential(scale=params["av_ID_duration"], size=numResetIndivs)
        )
        ID_periods[ID_periods == 0] = 1
        vals["Ind_ID_period_base"][reset_indivs] = ID_periods
        D_periods = np.round(
            np.random.exponential(scale=params["av_D_duration"], size=numResetIndivs)
        )
        D_periods[D_periods == 0] = 1
        vals["Ind_D_period_base"][reset_indivs] = D_periods

    vals["bact_load"][reset_indivs] = 0
    vals["treatProbability"][reset_indivs] = (
        drawTreatmentProbabilities(
            numResetIndivs, vals["MDA_coverage"], vals["systematic_non_compliance"]
        ),
    )
...
def Import_individual(vals, import_indivs, params, demog, distToUse="Poisson"):
    ...
    propAges[:-1] = np.exp(-ages[:-1] / demog["mean_age"]) - np.exp(
        -ages[1:] / demog["mean_age"]
    )
    propAges[-1] = 1 - np.sum(propAges[:-1])
    numImportIndivs = len(import_indivs)
    vals["Age"][import_indivs] = np.random.choice(
        a=ages, size=numImportIndivs, replace=True, p=propAges
    )
    ...
    if distToUse == "Poisson":
        vals["Ind_ID_period_base"][import_indivs] = np.random.poisson(
            lam=params["av_ID_duration"], size=numImportIndivs
        )
        vals["Ind_D_period_base"][import_indivs] = np.random.poisson(
            lam=params["av_D_duration"], size=numImportIndivs
        )
    else:
        ID_periods = np.round(
            np.random.exponential(scale=params["av_ID_duration"], size=numImportIndivs)
        )
        ID_periods[ID_periods == 0] = 1
        vals["Ind_ID_period_base"][import_indivs] = ID_periods
        D_periods = np.round(
            np.random.exponential(scale=params["av_D_duration"], size=numImportIndivs)
        )
        D_periods[D_periods == 0] = 1
        vals["Ind_D_period_base"][import_indivs] = D_periods
    vals["T_latent"][import_indivs] = 0
    vals["T_ID"][import_indivs] = (
        ID_period_function(import_indivs, params, vals) * np.random.uniform()
    )
...
def Check_and_init_MDA_treatment_state(params, vals, MDAData, numpy_state):
    ...
    np.random.set_state(numpy_state)
    if not set(
        ["treatProbability", "MDA_coverage", "systematic_non_compliance"]
    ).issubset(vals.keys()):
        MDA_coverage = 0
        treatProbability = np.full(shape=params["N"], fill_value=np.nan, dtype=float)
        systematic_non_compliance = params["rho"]
        ...
        if len(MDAData) > 0:
            MDA_coverage = MDAData[0][3]
            vals["treatProbability"] = drawTreatmentProbabilities(
                params["N"], MDA_coverage, systematic_non_compliance
            )
...
def YearlyBetaToWeeklyBeta(timesim, yearlyBetas):
    ...
    if extraPoints > 0:
        weeklyBetas = np.concatenate(
            (weeklyBetas, np.full(extraPoints, weeklyBetas[-1]))
        )  # Repeat final value
...
def check_if_we_need_to_redraw_probability_of_treatment(
    cov, systematic_non_compliance, vals
):
    if (cov != vals["MDA_coverage"]) | (
        systematic_non_compliance != vals["systematic_non_compliance"]
    ):
...
def update_MDA_information_for_output(
    MDAData,
    MDA_round_current,
    num_treated_people,
    vals,
    ...
    coverage[MDAData[MDA_round_current][-2]] += num_treated_people / np.count_nonzero(
        (vals["Age"] > ageStart * 52) & (vals["Age"] <= ageEnd * 52)
    )
    return nDoses, numMDA, coverage


def sim_Ind_MDA_Include_Survey(
    params,
    vals,
    timesim,
    burnin,
    ...
    doneSurveyThisYear = (
        False  # require this indicator so that we can output data for a survey even if
    )
    ...
    for i in range(timesim):
        ...
        if doIHMEOutput and i == nextOutputTime:
            ...
            results.append(
                outputResult(
                    ...
                    coverage,
                    numMDA - prevNMDA,
                    vals["nSurvey"] - vals["prevNSurvey"],
                    surveyPass,
                    true_elimination,
                    vals["numVacc"] - vals["prevNVacc"],
                    vals["nDosesVacc"],
                    vals["coverageVacc"],
                )
            )
        ...
        if i in MDA_times:
            ...
            for round_idx in range(len(MDA_round)):
                ...
                vals = check_if_we_need_to_redraw_probability_of_treatment(
                    cov, systematic_non_compliance, vals
                )
                # do the MDA for the age range specified by ageStart and ageEnd
                vals, num_treated_people = MDA_timestep_Age_range(
                    vals, params, ageStart, ageEnd, i / 52, label, demog
                )
                # keep track of doses and coverage of the MDA to be output later.
                nDoses, numMDA, coverage = update_MDA_information_for_output(
                    MDAData,
                    MDA_round_current,
                    num_treated_people,
                    vals,
        ...
        if i in vacc_times:
            ...
            if len(vacc_round) == 1:
                ...
                vals = vacc_timestep_Age_range(
                    params, vals, vacc_round, VaccData, i / 52, demog
                )
        ...
        vals = stepF_fixed(
            vals=vals, params=params, demog=demog, bet=betas[i], distToUse=distToUse
        )
        ...
        a, _ = np.histogram(
            vals["Age"], bins=max_age, weights=large_infection_count.astype(int)
        )
    ...
    vals["True_Prev_Disease_children_1_9"] = (
        prevalence  # save the prevalence in children aged 1-9
    )
    vals["True_Infections_Disease_children_1_9"] = (
        infections  # save the infections in children aged 1-9
    )
...
def returnSurveyPrev(
    vals, TestSensitivity, TestSpecificity, demog, t, surveyCoverage=1
):
    """
    ...
    positive = int(np.random.binomial(n=Diseased, size=1, p=TestSensitivity)) + int(
        np.random.binomial(n=NonDiseased, size=1, p=1 - TestSpecificity)
    )
    if t > 0:
        ...
        vals["n_surveys_population"][str(t) + ", surveys"] = n_people_by_age
        vals["n_surveys"][str(t) + ", surveys"] = n_surveys_by_age
    if surveyCoverage == 0:
        return 0, vals
    else:
...
def getResultsIHME(results, demog, params, outputYear):
    ...
    df = pd.DataFrame(
        0,
        range(len(outputYear) * 4 * 60 + len(outputYear) * 2),
    ...
    for i in range(len(results)):
        ...
        for j in range(len(d)):
            ...
            if len(Diseased) > 0:
                TruePositive = np.random.binomial(
                    n=1, size=len(Diseased[0]), p=params["TestSensitivity"]
                )
                pos[Diseased] = TruePositive
            if len(NonDiseased) > 0:
                FalsePositive = np.random.binomial(
                    n=1, size=len(NonDiseased[0]), p=1 - params["TestSpecificity"]
                )
                pos[NonDiseased] = FalsePositive

            Age = d[j].Age
            # Cast weights to integer to be able to count
            manyInfs, _ = np.histogram(
                Age, bins=max_age, weights=large_infection_count.astype(int)
            )
            Infs, _ = np.histogram(
                Age, bins=max_age, weights=infection_count.astype(int)
            )
            ...
            if i == 0:
                ...
                df.iloc[range(ind, ind + max_age), 3] = np.repeat(
                    "TruePrevalence", max_age
                )
                ...
                newrows = pd.DataFrame(
                    {
                        ...
                        "measure": np.repeat(
                            measure + " round " + str(MDAroundNumber), len(value)
                        ),
                        "draw_0": value,
                    }
                )
                ...
                newrows = pd.DataFrame(
                    {
                        ...
                        "age_end": range(1, 1 + int(max_age)),
                        "measure": np.repeat(
                            measure + " round " + str(MDAroundNumber) + " population",
                            len(value),
                        ),
                        "draw_0": value,
                    }
                ...
                newrows = pd.DataFrame(
                    {
                        ...
                        "age_end": range(1, 1 + int(max_age)),
                        "measure": np.repeat(
                            measure + " round " + str(VaccRoundNumber), len(value)
                        ),
                        "draw_0": value,
                    }
                )
                ...
                newrows = pd.DataFrame(
                    {
                        ...
                        "age_end": range(1, 1 + int(max_age)),
                        "measure": np.repeat(
                            measure + " round " + str(VaccRoundNumber) + " population",
                            len(value),
                        ),
                        "draw_0": value,
                    }
...
def combineIHME_MDA_SurveyData(
    results, demog, params, outputYear, Start_date, sim_params
):
...
def getResultsIPM(results, demog, params, outputYear, MDAAgeRanges, VaccAgeRanges):
    ...
    df = pd.DataFrame(
        ...
        range(
            len(outputYear) * 3
            + len(outputYear) * 3 * len(MDAAgeRanges)
            + len(outputYear) * 3 * len(VaccAgeRanges)
        ),
        columns=range(len(results) + 4),
    )
    df = df.rename(columns={0: "Time", 1: "age_start", 2: "age_end", 3: "measure"})
