...
def stepF_fixed(vals, params, demog, bet):
    ...
    vals["bact_load"][newInfectious] = bacterialLoad(
        newInfectious, params=params, vals=vals
    )
...
def getlambdaStep(
    params, Age, bact_load, IndD, bet, demog, vaccinated, time_since_vaccinated
):
...
def ID_period_function(newDis, params, vals):
    ...
    id_periods = (Ind_ID_period_base - params["min_ID"]) * np.exp(
        -params["inf_red"] * (No_Inf - 1)
    ) + params["min_ID"]
...
def D_period_function(Ind_D_period_base, No_Inf, params, Age):
    ...
    T_ID = np.round(
        params["min_D"]
        + (Ind_D_period_base - params["min_D"]) * np.exp(-aq * (No_Inf - 1) - ag * Age)
    )
...
def Reset_vals(vals, reset_indivs, params):
    ...
    vals["Ind_D_period_base"][reset_indivs] = (
        np.random.poisson(lam=params["av_D_duration"], size=numResetIndivs),
    )
    vals["bact_load"][reset_indivs] = 0
    vals["treatProbability"][reset_indivs] = drawTreatmentProbabilities(
...
def sim_Ind_MDA(
    ...
    for i in range(1, 1 + timesim):
        if i in MDA_times:
            ...
            for round_idx in range(len(MDA_round)):
                ...
                ageStart, ageEnd, cov, systematic_non_compliance = get_MDA_params(
                    MDAData, MDA_round_current, vals
                )
        ...
        prevalence_children_1_9.append(
            n_true_diseased_children_1_9 / n_children_ages_1_9
        )
        infections_children_1_9.append(
            n_true_infected_children_1_9 / n_children_ages_1_9
        )
    ...
    vals["True_Prev_Disease_children_1_9"] = (
        prevalence_children_1_9  # save the prevalence in children aged 1-9
    )
    vals["True_Prev_Infection_children_1_9"] = (
        infections_children_1_9  # save the infections in children aged 1-9
    )
    vals["True_Prev_Disease"] = (
        prevalence_All  # save the prevalence in children aged 1-9
    )
    vals["True_Prev_Infection"] = (
        infections_All  # save the infections in children aged 1-9
    )
...
def sim_Ind_MDA_Include_Survey(
    ...
    if doSurvey:
        surveyPrev = 0.5
        surveyPrev = returnSurveyPrev(
            vals, params["TestSensitivity"], params["TestSpecificity"]
        )
...
def getResultsIHME(results, demog, params, outputYear):
    ...
    df = pd.DataFrame(
        0, range(len(outputYear) * 4 * 60), columns=range(len(results) + 4)
    )
