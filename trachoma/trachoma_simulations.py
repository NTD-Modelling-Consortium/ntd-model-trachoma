...
def timer(func):
    ...
    def wrapper_timer(*args, **kwargs):
        ...
        print_function(
            f"-> Timer {run_uuid} running {func.__name__!r}, starting at {datetime.datetime.now()}"
        )
        value = func(*args, **kwargs)
        end_time = time.perf_counter()  # 2
        run_time = end_time - start_time  # 3
        print_function(
            f"=> Timer {run_uuid} finished {func.__name__!r} in {run_time:.4f} secs"
        )
        return value
...
def loadParameters(
    ...
    simparams = (
        pd.read_csv(BetFilePath)
        if numReps == 0
        else pd.read_csv(BetFilePath, nrows=numReps)
    )
    ...
    start_sim_year, end_sim_year = (
        int(timeparams.iloc[0, 0]),
        int(timeparams.iloc[0, 1]),
    )
    ...
    sim_times = [
        burnin + int(np.round(t))
        for t in np.arange(52 / 6, 52 * nyears + 52 / 6, 52 / 6)
    ]
    ...
    try:
        ...
        mda_dates = [
            pd.Timestamp(str(t[0]) + "-" + str(t[1]).zfill(2) + "-01")
            for t in mda_date_ints
        ]

        # number of weeks since 2020-01
        start_date_int = [2020, 1]
        month_addition = 1 if mda_date_ints[0][1] == 6 else 0
        weeks_from_202001 = int(
            ((mda_date_ints[0][0] - start_date_int[0]) * 52)
            + (((mda_date_ints[0][1] - start_date_int[1]) + month_addition) * (52 / 12))
        )
        ...
        try:
            ...
            mda_dates = pd.date_range(
                start=mda_start_date, end=mda_end_date, freq="12MS", normalize=True
            )
            mda_times = [
                burnin + int(np.round(t))
                for t in np.arange(
                    1 + 52 * (first_mda - start_sim_year), 52 * nyears + 52, 52
                )
            ]
    ...
    if VaccFilePath is not None:
        ...
        vacc_param_names = set(
            [
                "vaccination_date",
                "coverage",
                "prob_block_transmission",
                "reduce_bacterial_load",
                "reduce_duration",
                "waning_length",
            ]
        )
        ...
        if not vacc_param_names.issubset(vacc_params.keys()):
            raise ValueError(
                "Malformed vaccination file does not contain following parameters",
                vacc_param_names,
            )
    ...
    sim_params = dict(
        timesim=burnin
        + 52
        * nyears,  # years total duration of simulation (*52 so in weeks) including burn-in
...
def Trachoma_Simulation(
    ...
    if (
        ".csv" not in BetFilePath
        or ".csv" not in MDAFilePath
        or ".csv" not in PrevFilePath
        or (InfectFilePath is not None and ".csv" not in InfectFilePath)
    ):
        ...
        sim_params, params, demog = loadParameters(
            BetFilePath,
            MDAFilePath,
            PrevFilePath,
            InfectFilePath,
        ...
        if InSimFilePath is None:  # start new simulations
            ...
            def multiple_simulations(j):
                out = tf.sim_Ind_MDA(
                    ...
                    demog=demog,
                    bet=sim_params["Beta"][j],
                    MDA_times=sim_params["MDA_times"],
                    numpy_state=numpy_state,
                )
            ...
            if useCloudStorage is True and download_blob_to_file is not None:
                ...
                print_function(
                    f"Downloading pickle data (1) from {InSimFilePath} to {local_p_file}..."
                )
        ...
        print_function(
            f"Starting {numReps}x Trachoma runs on {num_cores} core(s), {model_version}"
        )

        start_time = time.time()

        out = Parallel(n_jobs=num_cores)(
            delayed(multiple_simulations)(j) for j in range(sim_params["n_sim"])
        )

        end_time = time.time()

        print_function(
            f"Finished {numReps}x Trachoma runs on {num_cores} core(s), {model_version}"
        )

        # save the simulated prevalence in a CSV file
        prevalence_columns = ["Random Generator", "bet"]
        prevalence_columns.extend(
            [d.strftime(format="%m-%Y") for d in sim_params["Out_dates"]]
        )
        ...
        for i in range(len(out)):
            ddf.iloc[i, 2:] = [
                out[i]["True_Prev_Disease_children_1_9"][j - 1]
                for j in sim_params["Out_times"]
            ]
        ...
        for i in range(len(out)):
            idf.iloc[i, 2:] = [
                out[i]["True_Infections_Disease_children_1_9"][j - 1]
                for j in sim_params["Out_times"]
            ]
        ...
        message = (
            "Running time: " + format((end_time - start_time), ".0f") + " seconds."
        )
